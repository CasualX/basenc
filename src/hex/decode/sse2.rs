...
pub unsafe fn decode(src: *const u8, mut len: usize, mut dest: *mut u8) -> Result<(), crate::Error> {
	let mut src = src as *const __m128i;
	while len >= 8 {
		let v1 = _mm_loadu_si128(src);
	...
	super::generic::decode(src as *const u8, len, dest as *mut u8)
}
