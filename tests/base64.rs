...
fn roundtrip(input: &[u8], encoding: &impl Encoding, expected: &str) {
	...
	assert_eq!(Ok(input), encoding.decode_into(expected, Vec::new()).as_deref());
}

#[track_caller]
fn error(string: &str, enc: &impl crate::Encoding, err: Error) {
	let mut buf = [0u8; 64];
	assert_eq!(enc.decode_into(string, &mut buf[..]), Err(err));
}
...
fn cwgem_test_base64_rb() {
	...
	error("^", &Base64StdStrict, Error::BadLength);
	error("A", &Base64StdStrict, Error::BadLength);
	error("A^", &Base64StdStrict, Error::BadLength);
	error("AA", &Base64StdStrict, Error::BadLength);
	error("AA=", &Base64StdStrict, Error::BadLength);
	error("AA===", &Base64StdStrict, Error::BadLength);
	error("AA=x", &Base64StdStrict, Error::InvalidChar);
	error("AAA", &Base64StdStrict, Error::BadLength);
	error("AAA^", &Base64StdStrict, Error::InvalidChar);
	error("AB==", &Base64StdStrict, Error::Denormal);
	error("AAB=", &Base64StdStrict, Error::Denormal);
